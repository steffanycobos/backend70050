...
productsRouter.get("/:pid", async (req, res) => {
  ...
  res.render("productDescription",  producto[0] );
});


// AGREGAR PRODUCTO
productsRouter.post("/", uploaderProducts.single('thumbnail'), async (req, res) => {
  let { title, description, price, code, stock, category } = req.body;
  let newProduct = await manager.addProducts(
    title,
    description,
    price,
    thumbnail,
...
productsRouter.put("/update/:pid", async (req, res) => {
  let pid = req.params.pid;
  let { title, description, price, code, stock, category } = req.body;
  let update = await manager.updateProduct(
    pid,
    title,
    description,
    price,
